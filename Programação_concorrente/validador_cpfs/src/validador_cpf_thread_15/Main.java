...
public class Main {
    public static void main(String[] args) {
        ...
        String saidaTempo = "src/tempos_execucao/tempo_execucao_cod15.txt";
        ...
        int arquivosPorThread = listaArquivos.size() / 15;

        for (int i = 0; i < 15; i++) {
            int inicioIndice = i * arquivosPorThread;
            int fimIndice = (i == 14) ? listaArquivos.size() : inicioIndice + arquivosPorThread;
            List<File> sublista = listaArquivos.subList(inicioIndice, fimIndice);
            Thread t = new Thread(new ProcessarCPFS(sublista, resultado));
            threads.add(t);
            t.start();
        }
